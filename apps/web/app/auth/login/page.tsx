...
function LoginContent() {
  ...
  async function handleGoogleLogin() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback?redirectTo=${redirectTo}` },
    })
  }
    ...
    <div className="min-h-screen bg-bg-base flex items-center justify-center px-4">
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <Link href="/auth/forgot-password" className="text-xs text-brand-secondary hover:text-brand-primary transition-colors">
                  Forgot password?
                </Link>
              </div>
        ...
        <p className="text-center text-sm text-text-muted mt-6">
          ...
          <Link href="/auth/register" className="text-brand-secondary hover:text-brand-primary transition-colors font-medium">
            Create one free
          </Link>
        </p>
      </div>
