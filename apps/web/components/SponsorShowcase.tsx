    ...
    <section className={`rounded-[2.25rem] border border-white/10 bg-[#101010] ${compact ? 'p-6 md:p-8' : 'p-8 md:p-10'}`}>
      ...
      <div className={`mt-8 grid gap-5 ${compact ? 'sm:grid-cols-2 xl:grid-cols-5' : 'sm:grid-cols-2 xl:grid-cols-5'}`}>
        {MAIN_SPONSORS.map(sponsor => {
          ...
          const cardClass = "flex h-full flex-col rounded-[1.75rem] border border-white/10 bg-white shadow-[0_18px_40px_rgba(0,0,0,0.18)] overflow-hidden transition-transform hover:scale-[1.02] hover:shadow-[0_24px_48px_rgba(0,0,0,0.24)]"
          return sponsor.url ? (
            <a key={sponsor.name} href={sponsor.url} target="_blank" rel="noopener noreferrer" className={cardClass}>
              {inner}
            </a>
          ) : (
